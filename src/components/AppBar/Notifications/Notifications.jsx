...
function Notifications() {
  ...
  const updateBoardInvitation = (status, invitationId) => {
    ...
    dispatch(updateBoardInvitationAPI({ status, invitationId }))
      .then(res => {
        const { boardInvitation } = res.payload // Lấy dữ liệu invitation từ response API
        // console.log(res)
        if (res.payload.boardInvitation.status === BOARD_INVITATION_STATUS.ACCEPTED) {
          // ✨ EMIT SOCKET: Thông báo User này đã tham gia Board
            socketIoInstance.emit('FE_USER_ACCEPTED_INVITATION', {
                boardId: boardInvitation.boardId,
                // Gửi thông tin User của người vừa chấp nhận (chính là user đang đăng nhập)
                newMember: currentUser 
            })
