...
function ToggleFocusInput({ value, onChangedValue, inputFontSize = '16px', ...props }) {
  ...
  // ✨ BƯỚC 1: SỬ DỤNG STATE ĐỂ THEO DÕI FOCUS (Cần thiết để ngăn lỗi khi đang gõ)
    const [isFocused, setIsFocused] = useState(false); // Khởi tạo state theo dõi focus

    useEffect(() => {
        // Chỉ cập nhật State nội bộ (inputValue) khi:
        // 1. Component KHÔNG ở trạng thái focus (để tránh gián đoạn khi user đang gõ).
        // 2. Giá trị Props (value) thay đổi (do Socket).
  ...
  return (
    <TextField
      ...
      onFocus={() => setIsFocused(true)} // ✨ BẮT SỰ KIỆN FOCUS
      onBlur={triggerBlur}
      {...props}
      // Magic here :D
      sx={{
