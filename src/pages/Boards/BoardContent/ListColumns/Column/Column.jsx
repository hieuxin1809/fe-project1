...
function Column({ column }) {
  ...
  const addNewCard = async () => {
    ...
    // ✨ EMIT SOCKET:
    if (board?._id) {
        socketIoInstance.emit('FE_NEW_CARD_CREATED', createdCard)
    }
  ...
  const handleDeleteColumn = () => {
    confirmDeleteColumn({
      ...
      const columnIdToRemove = column._id

      // Gọi API xử lý phía BE
      deleteColumnDetailsAPI(column._id).then(res => {
        toast.success(res?.deleteResult)
        if (board?._id) {
                socketIoInstance.emit('FE_COLUMN_REMOVED', {
                    boardId: board._id,
                    columnId: columnIdToRemove // Gửi ID của Column vừa bị xóa
                })
          }
      })
    }).catch(() => {})
  ...
  const onUpdateColumnTitle = (newTitle) => {
    ...
    updateColumnDetailsAPI(column._id, { title: newTitle }).then(() => {
      ...
      // ✨ EMIT SOCKET:
      if (board?._id) {
          // Gửi toàn bộ object column đã được cập nhật
          socketIoInstance.emit('FE_COLUMN_UPDATED', {
              _id: column._id, // Gửi ID Column
              boardId: board._id, // Gửi ID Board
              title: newTitle // Gửi Title mới
          })
      }
    })
  }
